...
class MetlifeAgent(BaseInsurerAgent):
    """Agent specialized in handling Metlife insurance contracts."""

    def __init__(self, file_path: str):
        """Initializes the agent with its contract file path."""
        super().__init__(insurer_name="metlife", model_name=INSURER_AGENT_MODEL)
        self.file_path = file_path
        self._contract_text = None
        self.executor = ThreadPoolExecutor(max_workers=1)

    def load_contract(self) -> str:
        """Loads the Metlife contract text from the corresponding data file."""
